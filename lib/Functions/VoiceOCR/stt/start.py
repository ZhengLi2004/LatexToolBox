...
app = Flask(__name__, static_folder=os.path.join(ROOT_DIR, 'static'), static_url_path='/static', template_folder=os.path.join(ROOT_DIR,
                                                                                                                             'templates'))
...
#        app.logger.error(f"[app]start error:{str(e)}")
    http_server = None
    http_server = WSGIServer(("127.0.0.1", 8001), app, handler_class=CustomRequestHandler)
    http_server.serve_forever()
